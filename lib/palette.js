...
// ## complementary
// Returns an array with two colors, generating the complementary
// color of the specified color.
palette.complementary = function (color) {
	return palette.shifted(color, 1, 180);
};
...
// ## complementaryTree
palette.complementaryTree = function (color, count, shift) {
	count = count || 4;
	shift = shift || 10;

	var comp = palette.complementary(color);

	return palette.tree(comp, count, shift);
};

// ## randomHueColor
// Generates a color from a random hue, with the specified
// saturation and brightness
palette.randomHueColor = function (saturation, brightness) {
	var hue = Math.round(Math.random() * 360),
		hexColor = new HexColor(0,0,0);

	hexColor.updateHsb([hue, saturation, brightness]);
	return hexColor;
}
